...
pub fn challenge(context: ChallengeContext) !void {
    ...
    const plaintext = try block.aes_cbc_decrypt(allocator, &[_]u8{0x00} ** 16, decoded, "YELLOW SUBMARINE");
...
test "Challenge 10" {
    ...
    const decoded_buffer = try allocator.alloc(u8, try base64.decodedLen(input.len, .standard));
    defer allocator.free(decoded_buffer);

    const decoded = try base64.decode(decoded_buffer, input, .standard);

    const plaintext = try block.aes_cbc_decrypt(allocator, &[_]u8{0x00} ** 16, decoded, "YELLOW SUBMARINE");
    defer allocator.free(plaintext);

    try std.testing.expectEqualStrings(expected_output, plaintext);
}
